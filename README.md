...
コメントの完了・完了取り消し（完了は削除せず一覧の最下部へ）、返信スレッド、優先度ラベル
（must / want / nit）、最終更新順と文書順の並び替え、全コメントをMarkdownで書き出す
「指摘をコピー」（AI修正指示にそのまま貼れる）、未保存コメントのlocalStorageへの
自動退避・復元も備える。イラスト付きの使い方ガイド（ドックの「❓ 使い方」で開く。自動表示はしない）
